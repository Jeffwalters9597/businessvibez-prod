-- Add or ensure video_url column exists on ad_designs table
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create index for video_url field if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
AS $$
BEGIN
  -- Sanitize image_url if it exists
  IF TG_OP = 'UPDATE' OR NEW.image_url IS NOT NULL THEN
    NEW.image_url := sanitize_url(NEW.image_url);
  END IF;
  
  -- Sanitize video_url if it exists
  IF TG_OP = 'UPDATE' OR NEW.video_url IS NOT NULL THEN
